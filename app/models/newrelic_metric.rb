class NewrelicMetric < Metric
  def fetch_value(start, finish)
    ...
    Newrelic.get_json("https://api.newrelic.com/api/v1/agents/#{agent.agent_id}/data.json?metrics[]=#{name}&field=#{field}&summary=1&begin=#{start.iso8601(0)}&end=#{finish.iso8601(0)}")
  end
