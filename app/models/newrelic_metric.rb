class NewrelicMetric < Metric
  def fetch_value(start, finish)
    Newrelic.get_json("https://api.newrelic.com/api/v1/agents/#{host.id}/data.json?metrics[]=#{name}&field=#{field}&summary=1&begin=#{start.iso8601(0)}&end=#{finish.iso8601(0)}")
  end
  ...
  def generate_sample(start=Time.now.utc-20.minutes, finish=Time.now.utc-10.minutes)
    ...
    if raw.nil? || raw[0].empty?
      puts "Error generating sample for #{host}:#{name}"
      return
    end
    value = raw[0][field]
    if maximum
